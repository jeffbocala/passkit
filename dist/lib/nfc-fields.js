...
class NFCField extends Array {
    constructor(nfcs) {
       ...
       */
        ...
        if (!Array.isArray(nfcs))
            throw new TypeError(`nfc must be an array, received ${typeof nfcs}: ${JSON.stringify(nfcs)}`);
        if (nfcs.length < 1)
            return;
        for (const { message, encryptionPublicKey } of nfcs) {
            if (encryptionPublicKey) {
                if (typeof encryptionPublicKey !== 'string')
                    throw new TypeError(`encryptionPublicKey must be Base64 encoded X.509 SubjectPublicKeyInfo structure containing a ECDH public key for group P256`);
                // this will check the PEM
                this.add(message, Buffer.from(encryptionPublicKey, 'base64').toString('utf8'));
            }
            else
                this.add(message);
        }
        ...
        return Array.from(this, ({ message, publicKey }) => ({
            ...
            encryptionPublicKey: publicKey
                ? // we need internal part of PEM message
                    Buffer.from(forge.pem
                        .decode(publicKey)
                        .find(({ type }) => type === 'PUBLIC KEY')
    ...
    add(message, publicKey) {
        if (typeof message !== 'string' || message.length > 64)
            throw new TypeError(`NFC message must be a string 64 characters or less, received "${message}"`);
        if (publicKey) {
            if (typeof publicKey !== 'string')
                throw new TypeError(`Public key must be a PEM encoded ECDH public key for group P256`);
